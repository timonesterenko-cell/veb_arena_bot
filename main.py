...
ОБНОВЛЕННАЯ ВЕРСИЯ с умными уведомлениями
"""
...
# Конфигурация
BOT_TOKEN = os.getenv("BOT_TOKEN", "YOUR_BOT_TOKEN_HERE")  # Токен из переменной окружения
...
class VebArenaBot:
    ...
    async def start_command(self, message: Message):
        ...
        welcome_text = (
            "Твой любимый написал это чудо, чтобы ты могла следить, когда можно купить пиву ❤️\n\n"
            "📝 Доступные команды:\n"
    ...
    async def status_command(self, message: Message):
        ...
        status_text = (
            ...
            f"📅 Уведомлений отправлено сегодня: {'Да' if datetime.now().date() in self.notified_dates else 'Нет'}\n"
    ...
    async def web_server(self):
        ...
        async def health_check(request):
            ...
            today_events = await self.get_events_for_today()
            ...
            return web.Response(
                text=f"✅ VEB Arena Bot is running!\n"
                     ...
                     f"🔄 Status: Active\n"
                     f"📤 Notified today: {'Yes' if datetime.now().date() in self.notified_dates else 'No'}",
        ...
        async def api_status(request):
            ...
            return web.json_response({
                ...
                'notified_today': datetime.now().date() in self.notified_dates,
    ...
    async def start(self):
        """Запуск бота"""
        logger.info("🚀 Запуск обновленного бота ВЭБ Арены...")
        ...
        logger.info("✅ Бот успешно запущен с умными уведомлениями")
...
def main():
    ...
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        ...
        print("📝 Создайте секрет BOT_TOKEN в Replit с токеном от @BotFather")
        return
