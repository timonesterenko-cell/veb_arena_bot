...
и уведомления о запрете продажи алкоголя
"""
...
class VebArenaBot:
    ...
    async def parse_veb_arena_events(self) -> List[Dict]:
        ...
        try:
            ...
            # Поиск блоков с мероприятиями
            # Структура может измениться, поэтому используем несколько селекторов
            event_selectors = [
                '.event-item',
                '.event-card', 
                '[class*="event"]',
            ...
            # Если не найдены стандартные селекторы, ищем по тексту
            if not event_elements:
                # Ищем по шаблону даты
...
def main():
    ...
    if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        ...
        print("📝 Откройте файл telegram_bot.py и замените YOUR_BOT_TOKEN_HERE на токен вашего бота")
        return
